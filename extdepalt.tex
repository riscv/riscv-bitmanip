...
much simpler {\tt shuffle} and {\tt unshuffle} instructions described below.
Those instructions simplify some of the most common cases that would otherwise
benefit from a fast \texttt{bext} and \texttt{bdep} unit. The hardware cost for
{\tt shuffle} and {\tt unshuffle} is comparably low for a processor that
already implements the {\tt grev} instruction.
...
return 0 in rd. Support for command 0 is mandatory. Command values 24-31 are
...
is omitted then $\texttt{N}=0$ is assumed for {\tt omega} and
$\texttt{N}=\log_2(\textrm{XLEN})-1$ is assumed for {\tt flip}):
