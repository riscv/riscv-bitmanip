...
0001 & zip + butterfly(mask, 1)                  & {\tt omega} \\
0010 & zip + butterfly(mask, 2)                  & {\tt omega} \\
0011 & zip + butterfly(mask, 3)                  & {\tt omega} \\
0100 & zip + butterfly(mask, 4)                  & {\tt omega} \\
0101 & zip + butterfly(mask, 5) (RV64/128; NSE)  & {\tt omega} \\
0110 & zip + butterfly(mask, 6) (RV128; NSE)     & {\tt omega} \\
...
uint_xlen_t shuffle(uint_xlen_t x, uint_xlen_t ctrl)
{
    ...
    if (dozip)
        return butterfly(zip(x), mask, stage);

    return butterfly(x, mask, stage);
}
\end{verbatim}
...
pseudo-instructions for rd $\neq$ rs and constant {\tt mask} (if {\tt N} is omitted
then {\tt N}$=$0 is assumed):
