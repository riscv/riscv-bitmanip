...
instructions can be emulated with XBitmanip using 3 instructions or less.
...
This instruction returns the index of the last set bit in {\tt rs}.
...
This instruction counts the number of consecutive 1’s or 0’s from MSB. If rs1 is 0, rD will be 0.

Using the arguably more consistent definition that the instruction should return XLEN when rs1 is 0,
and assuming {\tt rd} $\ne$ {\tt rs1}:
...
Simply add {\tt andi rd, rd, XLEN-1} if rd should be zero when rs1 is 0.
