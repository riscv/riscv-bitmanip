...
In order to avoid a situation where a platform needs to require the implementation of all bitmanip instructions when only some are needed, the bitmanip extension is subdividedd into smaller extensions, each of which has its own Z-extension mnemonic.
...
[preface]
== Pseudo-code for instruction semantics
