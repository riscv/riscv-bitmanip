...
In order to avoid a situation where a platform needs to require the implementation of all bitmanip instructions when only some are needed, the bitmanip extension is subdivided into smaller extensions, each of which has its own Z-extension mnemonic.
