...
\section{CRC instructions (\texttt{crc32[bhw], crc32c[bhw]})}

Unary CRC instructions that interpret the 32 LSB bits of rs1 as a
CRC32/CRC32C state and perform a polynomial reduction of that state shifted
left by 8, 16, or 32 bits. The instructions return the new CRC32/CRC32C state.
(On RV64 the result is sign extended to 64 bits.)
...
All 6 CRC instructions operate on bit-reflected data.

\input{bextcref-crc}

Payload data must be XOR'ed into the LSB end of the state before executing the
CRC instruction. The following code demonstrates the use of \texttt{crc32b}:

\begin{minipage}{\linewidth}
\begin{verbatim}
  uint32_t crc32_with_crc32b(const uint8_t *p, int len)
  {
...
implements a replacement for crc32w.
