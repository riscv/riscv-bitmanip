...
\begin{tikzpicture}
 ...
 \node (andc) [below=0.5cm of clz.west,anchor=west] { \texttt{andc, packw} };
 \node (cnot) [right=4.0cm of andc.west,anchor=west] { \texttt{c.not} };
 \node (slo) [below=0.5cm of andc.west,anchor=west] { \texttt{slo[i], sro[i]} };
...
\subsection{Pack 32-bit words (\texttt{packw}, RV64 only)}
...
This RV64-only instruction packs the 32-bit lower halves of rs1 and rs2 into
rd, with rs1 in the upper half and rs2 in the lower half.

\input{bextcref-packw}

Applications include 32-bit funnel shifts, zero-extend 32 bit values, duplicate the lower
32 bits (e.g. for mask creation), and loading unsigned 32 constants.

\begin{minipage}{\linewidth}
\begin{verbatim}
  ; Load 0xffff0000ffff0000
  lui rd, 0xffff0
  packw rd, rd, rd
\end{verbatim}
\end{minipage}
...
|  0000101    |   rs2   |   rs1   | 010 |    rd   |   0110011   |  PACKW
|---------------------------------------------------------------|
