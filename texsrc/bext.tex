...
\begin{tikzpicture}
 ...
 \node (cnot) [right=4.0cm of clz.west,anchor=west] { \texttt{c.not} };
 ...
 \node (andc) [below=1.0cm of min.west,anchor=west] { \texttt{andc, pack} };
 \node (rol) [below=0.5cm of andc.west,anchor=west] { \texttt{rol, ror[i]} };
 ...
 \begin{scope}[on background layer]
   ...
   \node (bperm) at ($ (andc.north west) + (+6.5cm,0) $) [above right, text=blue] { \textbf{Zbperm} };
   \draw [draw=black, fill=blue, opacity=0.2] ($ (andc.north west) + (-0.2cm,0) $) rectangle ($ (shuffle.south east) + (0,0) $);
...
shift operations {\tt op[26]=0}, except for immedate shifts on RV128 where {\tt op[26]}
contains the MSB of the immediate. (There is no FSLI/FSRI instruction.)
...
Only an encoding for RORI exists, as ROLI can be implemented with RORI by inverting
the immediate. Unary functions are encoded in the spot that would correspond to ROLI,
with the function encoded in the immediate.

The CRC instructions are encoded as unary instructions with {\tt op[24]} set. The
polynomial is selected with {\tt op[23]}, with {\tt op[23]=0} for CRC32 and
{\tt op[23]=1} for CRC32C. The width is selected with {\tt op[22:20]}, using
the same encoding as is used in {\tt funct3} for the load/store operations.

{\tt cmix} and {\tt cmov} is encoded using the two remaining ternary operator
...
The remaining instructions are encoded with {\tt funct7=0000100} or
{\tt funct7=0000101}.

The {\tt funct7=0000101} contains {\tt clmul[h]}, {\tt bmat[x]or},
and {\tt min[u]}/{\tt max[u]}.
...
The {\tt bmat[x]or} use the remaining slots in {\tt funct7=0000101} with {\tt
...
The remaining instructions are encoded with {\tt funct7=0000100}. The
shift-like {\tt shfl}/{\tt unshfl} instructions uses the same minor opcodes
as the shift operations. {\tt bdep} and {\tt bext} are encoded in a way so that
{\tt funct3[2]} selects the ``direction'', similar to shift operations.

RV64 has {\tt *W} instructions for all bitmanip instructions, with the
