...
\section{Unsigned address calculation instructions ({\tt addiwu, slliuw})}
...
{\tt addiwu} is identlical to {\tt addiw}, except that bits XLEN-1:32 of the
result are cleared after the addition. I.e. this instruction zero-extends instead
of sign-extends the 32-bit result.

\input{bextcref-addiwu}

{\tt slliuw} is identlical to {\tt slliw}, except that bits XLEN-1:32 of the
argument are cleared before the shift.

\input{bextcref-slliuw}

These two RV64-only instructions help in cases such as the following:
...
Using {\tt addiwu} and {\tt slliuw} the above C code translates to the
following RV64 assembler code:
...
The encoding of {\tt clmul[h[x]]} is identical to the encoding of {\tt mul[h[su]]},
...
{\tt bmat[x]or} uses the remaining slots in {\tt funct7=0000100} with {\tt
funct3=011} and {\tt funct3=111}.
...
|  0000100    |   rs2   |   rs1   | 011 |    rd   |   0110011   |  BMATXOR
