...
\begin{tikzpicture}
 ...
 \node (sbclr) [right=4.1cm of slo.west,anchor=west] { \texttt{sbclr[i]} };
 \node (sbinv) [right=4.1cm of min.west,anchor=west] { \texttt{sbinv[i]} };
 ...
 \node (orcb) [below=0.5cm of rev8.west,anchor=west] { \texttt{orc.b} };
 ...
 \begin{scope}[on background layer]
   ...
   \draw [draw=black, fill=red, opacity=0.2] ($ (clz.north west) + (-0.5cm,0) $) rectangle ($ (orcb.south west) + (3.5cm,0) $);
...
instructions. See Table~\ref{permbytes} for a list of those 24 sequences.
...
The bit field place ({\tt bfp}) instruction places up to 16 LSB bit from {\tt rs2} into
the value in {\tt rs1}. The upper bits of {\tt rs2} control the length of the bit field
and target position.

\input{bextcref-bfp}

The layout of the control word in rs2 is as follows. {\tt LEN=0} encodes for {\tt LEN=16}.
...
Placing bits from {\tt a0} in {\tt a1}, with results in {\tt t0}:
...
(On RV64 {\tt packw} would be used as second instruction in that sequence.)
...
Note that either above sequence only modifies one register, which makes them
fuse-able sequences.
...
|  0000100    |   rs2   |   rs1   | 111 |    rd   |   0110011   |  BFP
|---------------------------------------------------------------|
...
|  0000100    |   rs2   |   rs1   | 111 |    rd   |   0111011   |  BFPW
|---------------------------------------------------------------|
\end{verbatim}
\end{minipage}

Pending changes for v0.92 of the RISC-V Bitmanip Spec ({\tt +} for addition,
{\tt -} for removal):
...
The {\tt bfp} instruction is most commonly used in one of the following sequences:
...
Either sequence only reads at most two registers and only writes one register, making
them ideal candidates for macro-op fusion.
...
\subsection{Fused {\tt pack-}permute sequences}

A {\tt pack[hu]} instruction is often followed by a destructive permutation or
shift instruction with immediate ({\tt [un]shfli, rori, slli,} etc).

This makes it attractive to place the few MUXes for {\tt pack[hu]} instructions infront
of the shift unit, so that such sequences can be fused.
...
fusion described in Section~\ref{postfix-shift-fusion}.
