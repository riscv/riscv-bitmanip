...
\begin{tikzpicture}
 ...
 \node (sbs) [right=4.5cm of clz.west,anchor=west] { \texttt{sbs} };
 \node (sbr) [right=4.5cm of slo.west,anchor=west] { \texttt{sbr} };
 \node (sbc) [right=4.5cm of min.west,anchor=west] { \texttt{sbc} };
 \node (sbt) [right=4.5cm of addwu.west,anchor=west] { \texttt{sbt} };
 ...
 \begin{scope}[on background layer]
   ...
   \draw [draw=black, fill=gray, opacity=0.2] ($ (clz.north west) + (0,+0.2cm) $) |- ($ (clmul.south west) + (3.15cm,-0.2cm) $)
   ...
   \node (bs) at ($ (sbs.west) + (2.0cm,0) $) [right, text=blue] { \textbf{Zbs} };
   \node (bsingle) [below=-0.1cm of bs.south west,anchor=north west] { (single bit) };
   \draw [draw=black, fill=blue, opacity=0.2] ($ (sbs.north west) + (-0.2cm,0) $) rectangle ($ (sbt.south west) + (2.0cm,0) $);
   ...
   \draw [draw=black, fill=blue, opacity=0.2] ($ (andn.north west) + (-0.2cm,0) $) |- ($ (shuffle.south east) + (0,0) $)
     |- ($ (brevp.north west) + (2.5cm,0.2cm) $) |- ($ (andn.north west) + (-0.2cm,0) $);
   ...
   \node (bt) at ($ (cmov.west) + (3.0cm,0) $) [right, text=blue] { \textbf{Zbt} };
   ...
   \node (bm) at ($ (bmatxor.west) + (3.0cm,0) $) [right, text=red] { \textbf{Zbm} };
...
Endianness-swap pseudo-ops for {\tt c.sr[la]i} postfix fusion:

\begin{minipage}{\linewidth}
\begin{verbatim}
  eswap.b rd, rs    ->   bswap rd, rs; c.srai rd, (XLEN-8)
  eswap.h rd, rs    ->   bswap rd, rs; c.srai rd, (XLEN-16)
  eswap.w rd, rs    ->   bswap rd, rs; c.srai rd, (XLEN-32)   (OR   bswap[w] rd, rs   if applicable)
  eswap.d rd, rs    ->   bswap rd, rs; c.srai rd, (XLEN-64)   (OR   bswap[d] rd, rs   if applicable)

  eswapu.b rd, rs   ->   bswap rd, rs; c.srli rd, (XLEN-8)
  eswapu.h rd, rs   ->   bswap rd, rs; c.srli rd, (XLEN-16)
  eswapu.w rd, rs   ->   bswap rd, rs; c.srli rd, (XLEN-32)   (OR   bswap rd, rs   if applicable)
  eswapu.d rd, rs   ->   bswap rd, rs; c.srli rd, (XLEN-64)   (OR   bswap rd, rs   if applicable)
\end{verbatim}
\end{minipage}
