...
with open("synth.ys", "w") as f:
    for xlen in (32, 64):
        ...
        print("read_verilog -defer rvb_simple.v", file=f)
        print("hierarchy -top rvb_simple -chparam XLEN %d" % (xlen,), file=f)
        print("rename rvb_simple rvb_simple_xlen%d" % (xlen,), file=f)
