...
instruction sequences are titles ``macro''.

Pseudo-instructions that map to a sequence of more than one instruction are designed
so that they do not spill registers and, if possible, only read one or two registers
and only modify one register. They are also designed to prefer instructions for which
compressed encodings exist. This properties makes the pseudo-instructions defined in
this section prime candidates for macro-op fusion.
...
\subsection{MIX/MUX Pattern}

\subsubsection{MIX Macro}
...
\subsection{Bit scanning and counting}
...
\subsubsection{Permute-mask pseudo-instructions}

We define the following pseudo instructions for {\tt rd} $\ne$ {\tt rs2}.

\begin{verbatim}
  rorm rd, rs1, rs2, N     ->     rori rd, rs1, N
                                  and rd, rd, rs2

  grevm rd, rs1, rs2, N    ->     grevi rd, rs1, N
                                  and rd, rd, rs2

  gzipm rd, rs1, rs2, N    ->     gzip rd, rs1, N
                                  and rd, rd, rs2
\end{verbatim}

\subsubsection{Macros for butterfly operations}
...
\subsubsection{Macros for omega-flip networks}
...
\subsubsection{Macros for baseline networks}
...
\subsubsection{Macros for sheep-and-goats}
