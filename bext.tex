...
\begin{center}
\begin{tikzpicture}
 \node (clz) { \textbf{clz, ctz} };
 \node (pcnt) [below=0.5cm of clz.west,anchor=west] { \textbf{pcnt, andc} };
 \node (cnot) [right=4.0cm of pcnt.west,anchor=west] { \textbf{c.not} };
 \node (min) [below=0.5cm of pcnt.west,anchor=west] { \textbf{min[u], max[u]} };

 \node (rol) [below=1.0cm of min.west,anchor=west] { \textbf{rol, ror[i]} };
 \node (bswap) [below=0.5cm of rol.west,anchor=west] { \textbf{bswap, brev} };
 \node (pswap) [right=4.0cm of bswap.west,anchor=west] { \textbf{pswap} };
 \node (grev) [right=6.5cm of bswap.west,anchor=west] { \textbf{grev[i]} };

 \node (zip) [below=1.0cm of bswap.west,anchor=west] { \textbf{zip, unzip, brev.p} };
 \node (shuffle) [right=6.5cm of zip.west,anchor=west] { \textbf{shfl[i], unshfl[i]} };

 \node (bext) [below=1.0cm of zip.west,anchor=west] { \textbf{bext, bdep} };

 \node (clmul) [below=1.0cm of bext.west,anchor=west] { \textbf{clmul[h]} };

 \node (cmov) [right=3.5cm of bext.west,anchor=north west] { \textbf{cmov, cmix} };
 \node (fsl) [below=0.5cm of cmov.west,anchor=west] { \textbf{fsl, fsr} };

 \node (crc32) [below=1.0cm of clmul.west,anchor=west] { \textbf{crc32.[bhwd]} };
 \node (crc32c) [below=0.5cm of crc32.west,anchor=west] { \textbf{crc32c.[bhwd]} };

 \node (bitmatxor) [right=3.5cm of crc32.west,anchor=west] { \textbf{bmat[x]or} };
 \node (bitmatflip) [below=0.5cm of bitmatxor.west,anchor=west] { \textbf{bitmatflip} };
 ...
 \begin{scope}[on background layer]
   ...
   \node (bmini) at ($ (clz.north west) + (-0.5cm,0) $) [rotate=90, above left, text=red] { \textbf{Zbmini} };
   ...
   \node (bmat) at ($ (bitmatxor.west) + (3.0cm,0) $) [right, text=red] { \textbf{Zbmat} };
   \draw [draw=black, fill=red, opacity=0.2] ($ (bitmatxor.north west) + (-0.2cm,0) $) rectangle ($ (bitmatflip.south west) + (3.0cm,0) $);
 \end{scope}
\end{tikzpicture}
\end{center}
