...
on entry (preformatted mask on exit):
...
A sequence of $2 log_2(\textrm{XLEN})-1$ butterfly operations can perform any
...
But many permutations arising from real-world applications can be implemented
using shorted sequences. For example, any sheeps-and-goats operation with either
the sheeps or the goats bit reversed can be implemented in $log_2(\textrm{XLEN})$
...
can often be implemented using a much shorter sequence.
